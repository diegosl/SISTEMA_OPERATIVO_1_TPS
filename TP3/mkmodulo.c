...
#define  DEVICE_NAME "mkmodulo"    
#define  CLASS_NAME  "mkm"        

MODULE_LICENSE("GPL");            
MODULE_AUTHOR("Choquevilca & Sosa");    
MODULE_DESCRIPTION("Linux char driver encriptacion ");  
MODULE_VERSION("1.0");            
#define BUFFER 1000
...
static struct file_operations fops =
{
   ...
   static int __init mkmodulo_init(void){
   printk(KERN_INFO "mkmodulo: Inicializando el mkmodulo \n");

   
   majorNumber = register_chrdev(0, DEVICE_NAME, &fops);
   if (majorNumber<0){
      printk(KERN_ALERT "mkmodulo: no pudo registrar un número importante\n");
      return majorNumber;
   }
   printk(KERN_INFO "mkmodulo: ha sido registrado correctamente con el número principal %d\n", majorNumber);
   ...
   printk(KERN_INFO "mkmoduloDevice: clase de dispositivo registrada correctamente \n");
   ...
   printk(KERN_INFO "mkmoduloDevice: clase de dispositivo creada correctamente\n"); 
   return 0;
}


   static void __exit mkmodulo_exit(void){
   ...
   printk(KERN_INFO "mkmoduloDevice: el modulo se esta cerrando!\n");
}


static int dev_open(struct inode *inodep, struct file *filep){
   numberOpens++;
   printk(KERN_INFO "mkmodulo: se ha abierto el dispositivo %d tiempo(s)\n", numberOpens);
   return 0;
}
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   ...
   if (error_count==0){            
      printk(KERN_INFO "mkmoduloDevice: se ha enviado %d caracteres al usuario\n", size_of_message);
      return (size_of_message=0);  
   }
   else {
      printk(KERN_INFO "mkmoduloDevice: Error al enviar %d caracteres al usuario\n", error_count);
      return -EFAULT;             
   }
}
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   ...
   printk(KERN_INFO "mkmoduloDevice: ha recibido %zu caracteres del usuario\n", len);
   return len;
}


static int dev_release(struct inode *inodep, struct file *filep){
   printk(KERN_INFO "mkmoduloDevice: El dispositivo se ha cerrado\n");
   return 0;
}

module_init(mkmodulo_init);
module_exit(mkmodulo_exit);
